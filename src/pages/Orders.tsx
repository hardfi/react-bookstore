...
export function Orders(props: Props) {
  const {
    orders, user, books, history,
  } = props;
  const isAdmin = user.role === USER_ROLE.admin;
  const allOrders = isAdmin ? orders : orders
    .filter((order: OrderModel) => order.id.includes((user.id).toString()));
      ...
      <Grid container item xs={12}>
        <Grid container item justify='center' xs={12}>
          <Typography
            ...
            className='py-4'
          >
            {isAdmin ? 'Customer orders:' : 'Your previous orders with us:'}
          </Typography>
        </Grid>
        <Table aria-label='simple table'>
          ...
          <TableBody>
            {allOrders.map((order: OrderModel) => (
              ...
              <TableRow>
                <TableCell colSpan={5}>
                  <Typography
                    ...
                    align='center'
                  >
                    {isAdmin ? 'There\'s nothing to show' : 'Your don\'t have any past orders.'}
                  </Typography>
                </TableCell>
              </TableRow>
            )}
